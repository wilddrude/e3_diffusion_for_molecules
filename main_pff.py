...
try:
    ...
    pass
import build_pff_dataset  # Ueberarbeiten?
from configs.datasets_config import pff_with_h
import copy
import utils
import argparse
import wandb
...
parser.add_argument("--dataset", type=str, default="pff", help="dataset name")
parser.add_argument(
    "--filter_n_atoms",
    type=int,
    default=None,
...
data_file = "./data/pff/10K_mols.npy"

if args.remove_h:
    raise NotImplementedError()
else:
    dataset_info = pff_with_h

args.cuda = not args.no_cuda and torch.cuda.is_available()
device = torch.device("cuda" if args.cuda else "cpu")
dtype = torch.float32

split_data = build_pff_dataset.load_split_data(
    data_file,
    val_proportion=0.1,
    test_proportion=0.1,
...
for key, data_list in zip(["train", "val", "test"], split_data):
    dataset = build_pff_dataset.GeomDrugsDataset(data_list, transform=transform)
    shuffle = (key == "train") and not args.sequential

    # Sequential dataloading disabled for now.
    dataloaders[key] = build_pff_dataset.GeomDrugsDataLoader(
...
model, nodes_dist, prop_dist = get_model(
    args, device, dataset_info, dataloader_train=dataloaders["train"]
)
